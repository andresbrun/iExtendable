...
extension IceCream {
    ...
    init?(queryItems: [URLQueryItem]) {
        ...
        for queryItem in queryItems {
            ...
            if let decodedPart = Base(rawValue: value) where queryItem.name == Base.queryItemKey {
                base = decodedPart
            }
            if let decodedPart = Scoops(rawValue: value) where queryItem.name == Scoops.queryItemKey {
                scoops = decodedPart
            }
            if let decodedPart = Topping(rawValue: value) where queryItem.name == Topping.queryItemKey {
                topping = decodedPart
            }
        }
